#!/usr/bin/env python3
"""Pluggable credential storage for synthflow-mcp.
...
Storage order (read):  OS keyring  ->  process env  ->  ~/.synthflow-mcp/.env file
...
user opts out of keyring with ``SYNTHFLOW_MCP_USE_KEYRING=0``.
...
_USE_KEYRING_FLAG = "SYNTHFLOW_MCP_USE_KEYRING"
# ---------------------------------------------------------------------------
...
def _keyring_enabled() -> bool:
    ...
    Opt out with ``SYNTHFLOW_MCP_USE_KEYRING=0`` (or false/no/off). A failing or
